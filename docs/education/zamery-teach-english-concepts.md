...
```bash
npx skills add maemreyo/zmr-skills@zmr-dev --skill=zamery-teach-english-concepts
```
...
[Source](https://github.com/maemreyo/zmr-skills/tree/zmr-dev/skills/education/zamery-teach-english-concepts)
...
- **A concept model and prediction** — what the concept is, stated precisely for the learner's band and minimum approved ClassProfile or snapshot context.
- **A misconception contrast and counterexample** — the likely error, its nearest confusable, and evidence that tests the learner's initial model.
- **A board plan** — what goes on the board, how it builds across the teaching phase, and when each element appears.
- **Self-explanation, comprehension checks, and transfer** — a teach-back move that exposes reasoning, verifies understanding, and applies the concept in a new context.

The output is four things: teacher moves, expected learner thinking, the board plan, checks, and the transfer prompt. No more.

## Where it fits

`zamery-teach-english-concepts` is a **standalone teaching step** that can be used on its own or sequenced inside a larger flow. It comes after the blueprint and before practice, assessment, or slides. Its closest neighbour is [zamery-build-english-practice](https://aihero.dev/skills-zamery-build-english-practice), which turns the taught concept into drillable exercises. Use [zamery-teacher-copilot](https://aihero.dev/skills-zamery-teacher-copilot) for Zamery routing and [ask-matt](https://aihero.dev/skills-ask-matt) for the wider skill set.

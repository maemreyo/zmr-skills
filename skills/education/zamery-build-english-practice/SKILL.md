...
1. Recover the approved objective IDs, learner band, source lesson or concept, language, count, difficulty, and delivery constraints. Apply `../_shared/references/brief-version-contract.md` to the approved brief and every blueprint, concept, reteaching plan, snapshot, and authority dependency; reject stale or unapproved input even when invoked directly. When no blueprint exists, ask exactly one question only for a material gap.
...
3. Use `references/practice-progression.md` and `../_shared/references/learning-science-protocols.md`: worked example, faded guidance, independent practice, spaced retrieval, interleaving, and near/far transfer when applicable.
...
7. Accept targeted objective, misconception, scaffold, and reassessment requests from `zamery-plan-english-reteaching`. Use `../_shared/references/english-oral-language-playbook.md` for listening and speaking practice families.
8. Request DOCX or offline HTML projection only when asked. Route reusable pools to item banks, graded forms to assessment composition, IELTS profiles to IELTS practice, and linked-video work to video learning.
